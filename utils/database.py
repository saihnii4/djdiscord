...
class DJDiscordDatabaseManager:
    def __init__(
        self, rdbconn: rethinkdb.net.Connection, psqlconn: asyncpg.Connection
    ) -> None:
    ...
    async def _rethinkdb_execute(
        ...
        TableEvaluation, DatabaseEvaluation, DocumentEvaluation, dict, list
    ]:
        ...
        if isinstance(
            query, (rethinkdb.ast.Delete, rethinkdb.ast.Update, rethinkdb.ast.Insert)
        ):
    ...
    async def log(
        ...
        op: typing.Union[
            BeforeCogInvoke, AfterCogInvoke, BeforeCommandInvoke, AfterCommandInvoke
        ],
        info,
    ...
    async def get(self, **kwargs) -> list:
        ...
        return [
            obj
            async for obj in await rethinkdb.r.table(kwargs.get("table", "playlists"))
            .filter(kwargs)
            .run(self.rdbconn)
        ]
