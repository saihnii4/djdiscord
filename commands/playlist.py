...
class PlaylistCommands(discord.ext.commands.Cog):
    async def cog_before_invoke(self, ctx: DJDiscordContext) -> None:
        ...
        await ctx.database.log(BeforeCogInvoke(ctx.author, self, ctx.command, ctx.guild, ctx.channel),
                               {"cpu": psutil.cpu_percent(), "ram": memory_sample.used / memory_sample.total,
                                "disk": psutil.disk_usage("/")})
        await ctx.trigger_typing()

    async def cog_after_invoke(self, ctx: DJDiscordContext) -> None:
        memory_sample = psutil.virtual_memory()
        await ctx.database.log(AfterCogInvoke(ctx.author, self, ctx.command, ctx.guild, ctx.channel),
                               {"cpu": psutil.cpu_percent(), "ram": memory_sample.used / memory_sample.total,
                                "disk": psutil.disk_usage("/")})

    async def cog_command_error(self, ctx: DJDiscordContext, error: Exception) -> None:
        await ctx.database.log(Error(ctx.author, self, ctx.command, ctx.guild, ctx.channel),
                               {"cpu": psutil.cpu_percent(), "ram": memory_sample.used / memory_sample.total,
                                "disk": psutil.disk_usage("/")})

    @discord.ext.commands.group(name="playlist")
    async def playlist(ctx: DJDiscordContext) -> typing.Optional[discord.Message]:
    ...
    async def volume(ctx: DJDiscordContext, volume: VolumeConverter) -> None:
    ...
    async def skip(ctx: DJDiscordContext) -> typing.Optional[discord.Message]:
        if ctx.author.voice is None:
            return await ctx.send(
    ...
    async def loop(ctx: DJDiscordContext) -> typing.Optional[discord.Message]:
        if ctx.author.voice is None:
            return await ctx.send(
                "You need to be connected to a channel in order to skip music")
    ...
    async def stop(ctx: DJDiscordContext) -> typing.Optional[discord.Message]:
        if ctx.author.voice is None:
            return await ctx.send(
    ...
    async def delete(ctx: DJDiscordContext,
    ...
    async def add(ctx: DJDiscordContext, *,
    ...
    async def create(ctx: DJDiscordContext) -> typing.Optional[discord.Message]:
